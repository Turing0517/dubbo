 ...
 * ConditionRouter
 * 条件路由
 * 条件路由规则是一条字符串，对应Dubbo来说，它并不能直接理解字符串的意思，需要将其解析成内部格式才行
...
public class ConditionRouter extends AbstractRouter {
    ...
    public void init(String rule) {
        try {
            ...
            //将解析出的匹配规则分别赋值给whenCondition和thenCondition
    ...
    /**
     * 条件路由接口
     * route方法先是调用matcheWhen对服务消费者进行匹配，如果匹配失败，直接返回Invoker列表。如果匹配成功，再对服务提供者进行匹配，匹配
     * 逻辑封装在matchThen方法中。
    ...
    public <T> List<Invoker<T>> route(List<Invoker<T>> invokers, URL url, Invocation invocation)
        ...
        try {
            ...
            //服务提供者匹配条件为配置，表明对指定的服务消费者禁用服务
            if (thenCondition == null) {
                logger.warn("The current consumer in the service blacklist. consumer: " + NetUtils.getLocalHost() + ", service: " + url.getServiceKey());
                return result;
            }
            //这里可以简单把Invoker理解为服务提供者，现在使用服务提供者匹配规则对Invoker列表进行匹配
            ...
            //返回匹配结果，如果result为空列表，且force= true，表示强制返回空列表
            //否则路由结果为空的路由规则将自动失效
            if (!result.isEmpty()) {
                return result;
            } else if (force) {
...
//        return true;
    ...
    private boolean matchCondition(Map<String, MatchPair> condition, URL url, URL param, Invocation invocation) {
        ...
        for (Map.Entry<String, MatchPair> matchPair : condition.entrySet()) {
            //获取匹配项名称，比如host、method等
            String key = matchPair.getKey();
            String sampleValue;
            //invocation不为空，且key为methods表示进行方法匹配
    ...
    /**
     * MatchPair内部包含了两个Set类型的成员变量，分别用于存放匹配和不匹配的条件。这个类两个成员变量会在parseRule方法中被使用
     */
