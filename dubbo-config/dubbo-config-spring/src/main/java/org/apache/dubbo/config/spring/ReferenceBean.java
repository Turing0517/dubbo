 ...
 * Dubbo服务引用的时机有两个
 * 1.是在Spring容器调用ReferenceBean的afterProperties方法时引用服务
 * 2.是在ReferenceBean对应的服务被注入到其他类中是引用。这两个引用服务的时机区别在于，第一个是饿汉式的，第二个是懒汉式的。
 * 默认情况下，Dubbo使用懒汉式引用服务。如果需要饿汉式，可通过配置<dubbo:reference></dubbo:reference>的init属性开启
 */
public class ReferenceBean<T> extends ReferenceConfig<T> implements FactoryBean, ApplicationContextAware, InitializingBean, DisposableBean {
     ...
     * 该方法定义在Spring的FactoryBean接口中，ReferenceBean实现了这个方法
     * @return
     */
    @Override
