...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        // export service.
        //获取服务标识，理解成服务坐标也可以。由服务组名，服务名，服务版本号以及端口组成。比如：
        // demoGroup/com.alibaba.dubbo.demo.DemoService:1.0.1:20880
        String key = serviceKey(url);
        //创建DubboExporter
        DubboExporter<T> exporter = new DubboExporter<T>(invoker, key, exporterMap);
        //将<key,export>键值对放入缓存中
     ...
     * 创建服务器
     * 包含了三个核心逻辑。
     * 1.检测是否存在server参数所代表的Transporter拓展，不存在则抛出异常。
     * 2.创建服务器实例。
     * 3.检测是否支持client参数所表示的Transporter拓展，不存在也是抛出异常。
     * @param url
     * @return
     */
    private ExchangeServer createServer(URL url) {
        ...
        if (str != null && str.length() > 0) {
            //获取所有的Transporter实现类名称集合，比如：supportedType = [netty ,mina]
    ...
    /**
     * 获取客户端实例
     * 这里根据connections数量决定是获取共享客户端还是创建新的客户端实例，默认情况下，使用共享客户端实例。
     * @param url
     * @return
     */
     ...
     *  初始化客户端
     *  首先获取用户配置的客户端类型，默认为netty。然后检测用户配置的客户端类型是否存在，不存在则抛出异常。最后根据lazy配置决定创建
     *  什么类型的客户端。这里的LazyConnectionExchangeClient代码并不是很复杂，该类会在request方法被调用时，通过Exchangers的connect
     *  方法创建ExchangeClient的客户端
     * @param url
     */
